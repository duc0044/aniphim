...
AniPhim is an anime and animated movie streaming web app built with Nuxt and Tailwind CSS. It aggregates animation-focused movie data from the OPhim and NguonC APIs, normalizes the responses on the server, and presents the content with a Netflix-inspired sky-blue interface.

## Features

- Anime and animation catalogue from OPhim and NguonC.
...
- `/phim?type=series` - Browse anime series.
- `/phim?type=single` - Browse anime movies.
- `/phim?q=keyword` - Search anime and animation titles.
...
- Only animation titles are kept in the normalized result.

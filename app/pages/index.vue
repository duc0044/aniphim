...
const rows = computed(() => [
  ...
  },
  {
    ...
    to: '/phim',
    items: movies.value.slice().reverse().slice(0, 12),
  },
  {
    title: 'Đã Hoàn Thành',
    to: '/phim',
    items: movies.value.filter((movie: any) => {
      const ep = String(movie.episode || '').toLowerCase()
      return ep.includes('full') || ep.includes('hoàn') || ep.includes('trọn')
    }).slice(0, 12),
  },
])

useHead({
  title: 'AniPhim - Xem anime và hoạt hình',
  meta: [
    {
      name: 'description',
      content: 'Xem anime và phim hoạt hình Vietsub từ OPhim và NguồnC với giao diện duyệt phim hiện đại.',
    },
  ],
})
</script>
...
<template>
  <main class="min-h-screen overflow-hidden">
    ...
    <section v-if="hero"
      ...
      <div class="absolute inset-y-0 right-0 hidden w-1/2 bg-linear-to-l from-orange-950/18 to-transparent lg:block" />
      ...
      <div
        ...
        <div class="max-w-5xl">
          <p class="mb-2 text-xs font-semibold text-orange-400 sm:mb-3 sm:text-sm">
            Kho anime và hoạt hình
          </p>
          <h1 class="max-w-5xl text-2xl font-black leading-tight text-white drop-shadow-2xl sm:text-4xl lg:text-6xl">
            {{ hero.name }}
          </h1>
          <p class="mt-2 max-w-2xl text-sm text-orange-200 sm:mt-3 sm:text-lg">
            {{ hero.originName || 'Kho anime Vietsub mới cập nhật' }}
          </p>
          <div class="mt-3 flex flex-wrap gap-1.5 text-xs font-bold text-slate-100 sm:mt-5 sm:gap-2 sm:text-sm">
            ...
            <span v-if="hero.quality" class="rounded bg-orange-500 px-2 py-1.5 text-slate-950 sm:px-3 sm:py-2">{{
              hero.quality }}</span>
          ...
          <p class="mt-3 max-w-2xl text-xs leading-6 text-slate-100 sm:mt-5 sm:text-sm sm:leading-7">
            {{ hero.originName ? `${hero.originName}. ` : '' }}{{ hero.time || 'Anime và hoạt hình Vietsub mới cập nhật.' }}
          </p>
          <div class="mt-6 flex items-center gap-3 sm:mt-8 sm:gap-4">
            <NuxtLink :to="{ path: `/phim/${hero.slug}`, query: { source: hero.source } }"
              class="grid size-12 shrink-0 place-items-center rounded-full bg-orange-500 text-slate-950 shadow-2xl shadow-orange-950/50 transition hover:scale-105 hover:bg-white sm:size-16"
              aria-label="Xem ngay">
        ...
        <div
          ...
          <button type="button"
            class="hidden size-11 shrink-0 place-items-center rounded-full border border-white/15 bg-black/35 text-white backdrop-blur transition hover:bg-white/15 sm:size-12 sm:grid"
            aria-label="Phim trước" @click="previousHero">
            <ChevronLeft class="size-5 sm:size-6" />
          </button>

          <div class="no-scrollbar flex gap-3 overflow-x-auto pb-2 sm:gap-4">
            <button v-for="(slide, index) in heroSlides" :key="`${slide.source}-${slide.slug}-thumb`" type="button"
              class="relative h-16 w-28 shrink-0 overflow-hidden rounded-lg border bg-slate-900 shadow-lg shadow-black/30 transition sm:h-20 sm:w-40"
              :class="index === heroIndex ? 'border-orange-500 ring-2 ring-orange-500/45' : 'border-white/15 opacity-70 hover:opacity-100'"
              @click="selectHero(index)">
              <img :src="slide.thumb || slide.poster" :alt="slide.name" class="h-full w-full object-cover">
              <span
                class="absolute inset-x-0 bottom-0 truncate bg-black/55 px-2 py-1 text-[11px] font-semibold text-white sm:text-xs">
                {{ slide.name }}
              </span>
            </button>
          </div>
    ...
    <section v-else class="mx-auto flex min-h-screen max-w-7xl items-center px-4 pt-36 sm:px-6 lg:px-8 lg:pt-24">
      <div>
        <h1 class="text-4xl font-black text-white">AniPhim</h1>
        <p class="mt-3 text-slate-300">Đang tải kho anime...</p>
      </div>
    </section>
    ...
    <div class="relative z-10 mx-auto -mt-10 max-w-390 px-4 pb-16 sm:px-6 lg:px-8 xl:px-10">
      ...
      <!-- <section v-if="watchHistory.length" class="mb-12">
        <div class="mb-4 flex items-center justify-between gap-4">
          ...
          <button type="button"
            class="grid size-10 shrink-0 place-items-center rounded-full border border-white/20 bg-white/8 text-white transition hover:border-orange-500/60 hover:bg-white/14"
        ...
        <div class="no-scrollbar -mx-4 ml-0.5 flex snap-x gap-4 overflow-x-auto px-4 pb-5 sm:ml-0">
          <NuxtLink v-for="item in watchHistory" :key="`${item.source}-${item.slug}`" :to="watchHistoryLink(item)"
            ...
            <div
              class="relative aspect-2/3 overflow-hidden rounded-md bg-slate-900 shadow-xl shadow-black/25 ring-1 ring-white/10 transition duration-300 group-hover:-translate-y-1 group-hover:ring-orange-500/60">
              <img :src="item.thumb || item.poster" :alt="item.name"
                class="h-full w-full object-cover transition duration-500 group-hover:scale-105">
              <div class="absolute inset-x-0 bottom-0 h-1 bg-white/18">
                <span class="block h-full w-1/3 bg-orange-500" />
              </div>
              <span
                class="absolute left-2 top-2 rounded bg-orange-500 px-2 py-1 text-xs font-black text-slate-950">
      ...
      <section v-for="(row, index) in rows" v-show="row.items.length"
        ...
        <div class="mb-4 flex items-end justify-between gap-4">
          ...
          <NuxtLink :to="row.to" class="text-sm font-semibold text-orange-400 hover:text-white">
            Xem thêm
          </NuxtLink>
        </div>

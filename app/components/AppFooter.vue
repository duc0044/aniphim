  ...
  <footer class="border-t border-white/10 bg-[#07090f] text-white">
    <div class="mx-auto max-w-[1560px] px-4 py-10 sm:px-6 lg:px-8 xl:px-10">
      <div class="flex flex-col gap-8 lg:flex-row lg:items-center">
        ...
        <div class="flex flex-wrap gap-3">
          <button
            ...
            :key="item.label"
            type="button"
            class="grid size-12 place-items-center rounded-full bg-orange-400 text-slate-950 shadow-lg shadow-sky-950/30 transition hover:-translate-y-0.5 hover:bg-white"
      ...
      <div class="mt-10 grid gap-8 text-sm font-bold sm:grid-cols-3 lg:max-w-3xl">
        <div v-for="(group, index) in linkGroups" :key="index" class="space-y-5">
          <NuxtLink
            ...
            :key="link"
            to="/phim"
            class="block text-orange-400 transition hover:text-white"
          >
            {{ link }}
          </NuxtLink>
        </div>
      ...
      <p class="mt-10 max-w-4xl text-sm leading-7 text-slate-300">
        ANIPHIM - Trang xem phim hoạt hình và anime online Vietsub với dữ liệu tổng hợp từ OPhim và NguồnC. Kho phim được làm lại theo hướng series, movie và tập mới cập nhật.
      </p>

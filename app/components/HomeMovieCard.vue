...
const languageBadge = computed(() => {
  const value = String(props.movie.lang || '').toLowerCase()
  if (!value) return ''
  if (value.includes('lồng tiếng') || value.includes('long tieng')) return 'Lồng tiếng'
  if (value.includes('thuyết minh') || value.includes('thuyet minh')) return 'Thuyết minh'
  if (value.includes('vietsub') || value.includes('phụ đề việt') || value.includes('phu de viet')) return 'Vietsub'
  return props.movie.lang || ''
})
...
<template>
  <NuxtLink :to="movieLink"
    ...
    <div
      class="absolute inset-0 overflow-hidden rounded-md bg-slate-900 shadow-xl shadow-black/25 ring-1 ring-white/10 transition duration-300 group-hover:ring-orange-500/60">
      ...
      <div class="absolute inset-x-0 bottom-0 bg-linear-to-t from-slate-950 via-slate-950/70 to-transparent p-3">
        ...
        <p class="mt-1 truncate text-xs text-orange-200">{{ movie.episode || movie.year || movie.quality }}</p>
      </div>
      <span class="absolute left-2 top-2 rounded bg-orange-500 px-2 py-1 text-xs font-black text-slate-950">
        {{ movie.source }}
      </span>
      <span v-if="languageBadge"
        class="absolute right-2 top-2 rounded bg-black/70 px-2 py-1 text-[11px] font-black text-white ring-1 ring-white/10">
        {{ languageBadge }}
      </span>
    </div>
  </NuxtLink>
  ...
  <Teleport to="body">
    <Transition enter-active-class="transition duration-180 ease-out" enter-from-class="scale-95 opacity-0"
      ...
      <NuxtLink v-if="isPreviewVisible" :to="movieLink"
        class="fixed z-90 hidden origin-top-left overflow-hidden rounded-xl bg-[#1a0b14] text-white shadow-2xl shadow-black/60 ring-1 ring-orange-500/45 sm:block"
        ...
        <div class="p-4 pt-3">
          <div class="grid grid-cols-[1fr_6.25rem_6.25rem] gap-2">
            <span
              class="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-orange-500 px-4 text-sm font-black text-slate-950 transition hover:bg-white">
          ...
          <div class="mt-3 flex flex-wrap gap-2 text-xs font-black">
            <span v-if="movie.quality" class="rounded bg-black/28 px-2.5 py-1.5 text-white ring-1 ring-white/10">{{
              movie.quality }}</span>
            <span v-if="movie.lang" class="rounded bg-orange-500 px-2.5 py-1.5 text-slate-950">{{ movie.lang }}</span>
          ...
          <p class="mt-3 line-clamp-2 text-xs font-bold leading-6 text-slate-200">
            ...
            <template v-if="!movie.countries?.length && !movie.categories?.length">Anime và hoạt hình mới cập nhật</template>
          </p>
        </div>
      </NuxtLink>
    </Transition>
